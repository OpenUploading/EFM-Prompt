"""DAMFNet binary EEG-HbR fusion baseline on FineMI."""
...
from finemi_damf_data import EEG_ROOT, FNIRS_ROOT, TASKS, load_split
from models.shin_damfnet import SHINDAMFNet


OUTPUT = Path(__file__).resolve().parent / "runs_finemi"


def arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="DAMFNet on paired FineMI EEG/HbR")
    parser.add_argument("--eeg-root", type=Path, default=EEG_ROOT)
    parser.add_argument("--fnirs-root", type=Path, default=FNIRS_ROOT)
    parser.add_argument("--output-root", type=Path, default=OUTPUT)
    parser.add_argument("--output-dir", type=Path)
    parser.add_argument("--cache-dir", type=Path)
    parser.add_argument("--task", choices=("mi",), default="mi")
    parser.add_argument("--train-subjects", default="1-12")
    parser.add_argument("--val-subjects", default="13-15")
    parser.add_argument("--test-subjects", default="16-18")
    ...
    parser.add_argument("--epoch-stop-s", type=float, default=4.0)
...
def model_factory(*, dropout: float, sensor_layout: str) -> SHINDAMFNet:
    return SHINDAMFNet(dropout=dropout, sensor_layout=sensor_layout, num_classes=2,
                       eeg_input_nodes=62, hbr_input_nodes=24)
...
def main() -> None:
    ...
    shared.SHIN_EEG_CHANNELS = [f"FineMI-EEG-{index:02d}" for index in range(1, 63)]
    shared.main()
