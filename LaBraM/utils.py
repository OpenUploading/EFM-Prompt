...
try:
    ...
    def binary_metrics_fn(target, output, metrics, threshold=0.5):
        ...
        if "balanced_accuracy" in metrics:
            results["balanced_accuracy"] = balanced_accuracy_score(target, pred)
        if "pr_auc" in metrics:
            results["pr_auc"] = average_precision_score(target, output)
        if "roc_auc" in metrics:
            results["roc_auc"] = roc_auc_score(target, output)
        return results

    def multiclass_metrics_fn(target, output, metrics):
        target = np.asarray(target).reshape(-1)
        ...
        if "cohen_kappa" in metrics:
            results["cohen_kappa"] = cohen_kappa_score(target, pred)
        if "f1_weighted" in metrics:
            results["f1_weighted"] = f1_score(target, pred, average="weighted")
        return results
import pandas as pd
